...
export const DropdownMenu = ({ trigger, items, size = 'md', side, align,label }: DropdownMenuProps) => {
  ...
  return (
    <Dropdown.Root onOpenChange={(open) => {
        if (!open) {
      ...
      <Dropdown.Portal>
        <Dropdown.Content
          className={clsx(sizeClass[size], "bg-white dark:bg-gray-800 rounded shadow-lg")}
          side={side}
          align={align}
        ...
        >
          <div className='relative flex justify-center items-center px-2 py-3'>
            <span>{label}</span>
            {history.length > 1 && <Button onClick={() => goBack()} className='absolute left-1' variant='transparent' icon={<ChevronLeft size={15}/>}/>}
          </div>
